...
\begin{abstract}
We describe our submission to SemEval-2026 Task 6: CLARITY, which aims to classify political question–answer pairs by response clarity and evasive technique. We investigate several approaches, including long-context transformers, multiple instance learning, hierarchical multi-task models, and a natural language inference (NLI) formulation. The NLI model achieves the best performance on Subtask 1 (macro-F1 0.81), while an attention-based MIL model performs best on Subtask 2 (macro-F1 0.45). Our findings demonstrate the benefits of entailment-based modeling for clarity prediction and localized reasoning for evasion detection under limited computational resources.
\end{abstract}
